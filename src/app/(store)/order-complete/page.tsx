        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-3 pt-2">
            <button
              onClick={() => {
                const stored = localStorage.getItem('nca_email') ?? ''
                if (stored) { setEmail(stored); startPolling(stored) }
                else setStatus('waiting-email')
              }}
...
function MetaField({ label, value, shortValue, align = 'left' }: {
  ...
  return (
    <div className={align === 'right' ? 'text-right' : ''}>
      <span className="font-mono text-[9px] uppercase tracking-widest text-text-muted block mb-0.5">{label}</span>
